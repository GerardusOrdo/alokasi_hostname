...
import '../widgets/dc_owner_table_widget.dart';
import 'dc_owner_filter_page_tab.dart';
import 'dc_owner_input_page_tab.dart';

class DcOwnerMainPage extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return GetBuilder<DcOwnerPloc>(
      // initState: (_) => Get.find<DcOwnerPloc>().getDcOwnerTableWidget(),
      builder: (_) => Scaffold(
            appBar: MasterPageAppBar(
              ploc: _,
              height: 70.0,
              filterPage: DcOwnerFilterPageTab(),
            ),
            floatingActionButton: MasterPageFABAddData(
              ploc: _,
              inputPage: DcOwnerInputPageTab(),
            ),
